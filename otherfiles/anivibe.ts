...
const fetchAniVibeSources = async (animeId: string, dub: boolean) => {
  ...
  if (cachedSources) {
    const filteredCachedSources = cachedSources?.sources?.filter((source) =>
      dub ? source?.type === "dub" : source?.type === "sub"
    );
    return filteredCachedSources?.length > 0
      ? { sources: filteredCachedSources }
      : null;
  }
  ...
  try {
    ...
    const scriptTag = $("script")
      .filter((_, el) => $(el).html()?.includes("loadIframePlayer"))
      .first();
