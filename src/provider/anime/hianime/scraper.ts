...
import type { Anime, AnimeInfo, EpisodeInfo, language, ScrappedServers } from './types.js';
...
export function extractSearchResults($: cheerio.CheerioAPI, selector: cheerio.SelectorType) {
  ...
  const currentPage: number | null =
    Number($(paginationElement).find('.active .page-link').text().trim() || 1) || null;
  const totalPages: number | null =
    Number(paginationElement.find('a.page-link[title="Last"]').attr('href')?.split('page=').at(-1) || 1) ||
    null;
...
export function extractAnimeInfo($: cheerio.CheerioAPI) {
  ...
  res.totalEpisodes = Number(
    section?.find('.tick .tick-item.tick-eps')?.text().trim() || res.episodes.sub || null,
  );
...
export function extractAnimeServerId($: cheerio.CheerioAPI, servernumber: Number, category: language) {
  return (
