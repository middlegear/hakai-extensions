...
export class AnimekaiDecoder {
  ...
  async decode(n: string): Promise<string | null> {
    const megaKeysSrc = await AnimekaiDecoder.loadMegaKeys();
    if (!megaKeysSrc) {
      console.error('Failed to load Mega keys for decoding.');
      return null;
    }
  ...
  static async loadMegaKeys(): Promise<string[] | null> {
    const Autokai = 'https://raw.githubusercontent.com/amarullz/kaicodex/refs/heads/main/generated/keys.json';
    try {
      const response = await axios.get(Autokai);

      const data = await response.data;
      return data?.mega || null;
    } catch (error) {
      console.error('Error loading Mega keys:', error);
      return null;
    }
  }
}
