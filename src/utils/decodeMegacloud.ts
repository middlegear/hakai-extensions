...
export class Decrypter {
  ...
  // New method to generate mapping key using secret and nonce
  private generateMappingKey(): string {
    // Combine secret and nonce to form the base string
    let baseString = this.secret + this.nonce;
    // Compute hash (not used in final output, kept for fidelity)
    let hash = 0n;
    for (let i = 0; i < baseString.length; i++) {
      hash = (hash * 31n + BigInt(baseString.charCodeAt(i))) & 0xffffffffn;
    }
    // Transform base string: XOR with 179
    const transformed: string[] = [];
    for (let i = 0; i < baseString.length; i++) {
      transformed.push(String.fromCharCode(baseString.charCodeAt(i) ^ 179));
    }
    baseString = transformed.join('');
    // Rotate by 7 positions
    const rotateIndex = 7; // Hardcoded as (0 % baseString.length) + 7
    baseString = baseString.slice(rotateIndex) + baseString.slice(0, rotateIndex);
    // Reverse the nonce
    const reversedNonce = this.nonce.split('').reverse().join('');
    // Combine baseString and reversedNonce
    let result = '';
    for (let i = 0; i < Math.max(baseString.length, reversedNonce.length); i++) {
      const char1 = baseString[i] || ' '; // Fallback to space (ASCII 32)
      const char2 = reversedNonce[i] || ' '; // Fallback to space (ASCII 32)
      result += char1 + char2;
    }
    // Map to printable ASCII (32–126)
    result = [...result].map(char => String.fromCharCode((char.charCodeAt(0) % 95) + 32)).join('');
    return result;
  }
  ...
  public decrypt(encrypted: string, iterations: number = 3): string {
    ...
    // Use the generated mapping key
    const keyphrase = this.generateMappingKey();
